...
# このスクリプトは、Aratako/T5Gemma-TTS-2b-2bモデルの学習の際に使用されたハイパーパラメータなどと同じ設定で、
# exampleデータ（この例ではEmilia-YODASの一部）を用いてスクラッチの学習を行う例です。（GPUは8基想定）
# This script performs training from scratch using example data (in this case, a portion of Emilia-YODAS)
# with the same hyperparameters used for training the Aratako/T5Gemma-TTS-2b-2b model. (It assumes 8 GPUs.)
