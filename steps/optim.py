...
class ScaledAdam(BatchedOptimizer):
    ...
    def _init_state(self, group: dict, p: Tensor, state: dict):
        ...
        numel_per_param = p.numel() // batch_size

        if numel_per_param > 1:
    ...
    def _step_one_batch(
        ...
        if clipping_scale != 1.0:
            # avoid allocating an extra grad buffer
            grad.mul_(clipping_scale)
        step = state["step"]
        delta = state["delta"]
    ...
    def _size_update(
        ...
        delta.addcmul_(p, scale_step, value=(1 - beta1))
    ...
    def _step(self, group: dict, p: Tensor, state: dict):
        ...
        denom += eps
        # in-place to avoid a large temporary buffer
        grad.div_(denom)

        alpha = -lr * (1 - beta1) * state["param_rms"].clamp(min=param_min_rms)

        delta = state["delta"]
        delta.addcmul_(grad, alpha)
        p.add_(delta)
