...
class StatefulSampler(Sampler):
    ...
    def set_epoch(self, epoch):
        ...
        if self.use_random:
            ...
            g = torch.Generator()
            g.manual_seed(self.seed + self.epoch)
            self.indices = torch.randperm(self.data_source_length, generator=g).tolist()
        else:
...
class DistributedDynamicBatchSampler(Sampler):
    ...
    Length for each example must be provided when instantiating this class via
    the lengths_list argument, which should contain one length per example in
    the dataset.
    ...
    >>> bsampler = DynamicBatchSampler(dataset, 20, 4, shuffle=False, batch_ordering='descending')
    ...
    lengths_list: list
        List containing the length of each example in the dataset. This argument
        is required and should align with the dataset ordering.
    epoch : int
    ...
    def _generate_batches(self):
        ...
        if self._epoch == 0:  # only log at first epoch
            ...
            if top_k > 0:
                ...
                self._batches = sorted_batches[:top_k] + self._batches[top_k:]
                if (not dist.is_initialized()) or dist.get_rank() == 0:
                    logging.info("replace the first %d samples in the batch with the %d longest samples: ", top_k, top_k)
                    logging.info("their lengths: ")
